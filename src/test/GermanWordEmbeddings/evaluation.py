...
# Adapted by Sebastian Gerard to fit the purpose of our paper
...
TARGET_SYN     = evalPath + 'data/syntactic.questions'
TARGET_SEM_OP  = evalPath + 'data/semantic_op.questions'
TARGET_SEM_BM  = evalPath + 'data/semantic_bm.questions'
TARGET_SEM_DF  = evalPath + 'data/semantic_df.questions'
SRC_NOUNS      = evalPath + 'src/nouns.txt'
SRC_ADJECTIVES = evalPath + 'src/adjectives.txt'
SRC_VERBS      = evalPath + 'src/verbs.txt'
SRC_BESTMATCH  = evalPath + 'src/bestmatch.txt'
SRC_DOESNTFIT  = evalPath + 'src/doesntfit.txt'
SRC_OPPOSITE   = evalPath + 'src/opposite.txt'
PATTERN_SYN = [
...
def test_mostsimilar(model, src, label='most similar', topn=10):
    ...
    # log result
    results.append([correct_matches, topn_matches])
    #results.append(", ".join([label, 'correct', str(correct_matches), str(num_right), str(num_questions)]))
    #results.append(", ".join([label, 'top ' + str(topn), str(topn_matches), str(num_topn), str(num_questions)]))
    #results.append(", ".join([label, 'coverage', str(coverage), str(num_questions), str(num_lines)]))
...
def test_mostsimilar_groups(model, src, topn=10):
    ...
    for group in groups:
        ...
        # log result
        #results.append(", ".join([label, 'correct', str(correct_group_matches), str(num_group_right), str(num_group_questions)]))
        #results.append(", ".join([label, 'top ' + str(topn), str(topn_group_matches), str(num_group_topn), str(num_group_questions)]))
        #results.append(", ".join([label, 'coverage', str(group_coverage), str(num_group_questions), str(num_group_lines)]))
...
def test_doesntfit(model, src):
    ...
    coverage = round(num_questions/float(num_lines)*100, 1) if num_lines>0 else 0.0
    # log result
    results.append(correct_matches)
    #results.append(", ".join(['doesn\'t fit', 'correct', str(correct_matches), str(num_right), str(num_questions)]))
    #results.append(", ".join(['doesn\'t fit', 'coverage', str(coverage), str(num_questions), str(num_lines)]))
...
def evaluate(modelpath, umlauts = True, topn = 10, binary = False):
	# get trained model
	model = gensim.models.KeyedVectors.load_word2vec_format(modelpath.strip(), binary = binary)
	# execute evaluation
	results = list()
	logging.info('> EVALUATING SYNTACTIC FEATURES')
	resGroups = test_mostsimilar_groups(model, TARGET_SYN + '.nouml' if umlauts else TARGET_SYN, topn)
	logging.info('> EVALUATING SEMANTIC FEATURES')
	resOpposite = test_mostsimilar(model, TARGET_SEM_OP + '.nouml' if umlauts else TARGET_SEM_OP, 'opposite', topn)
	resBestMatch = test_mostsimilar(model, TARGET_SEM_BM + '.nouml' if umlauts else TARGET_SEM_BM, 'best match', topn)
	resDoesntFit = test_doesntfit(model, TARGET_SEM_DF + '.nouml' if umlauts else TARGET_SEM_DF)
	return [(resGroups + resOpposite + resBestMatch), resDoesntFit]

def evaluateSingleResult(modelpath, umlauts = True, topn = 10, binary = False):
	results = evaluate(modelpath, umlauts, topn, binary)
	#results = [float(singleResult.split(', ')[2]) for singleResult in results]
	
	meanRes = [0.0, 0.0]
	for res in results[0]:
		meanRes[0] += res[0]
		meanRes[1] += res[1]
	meanRes[0] += results[1][0]
	meanRes[0] = meanRes[0]/(len(results[0])+1)
	meanRes[1] = meanRes[1]/len(results[1])
	return meanRes
...
if __name__ == "__main__":
	...
	parser.add_argument('result', type=str, help='result file containing results of the tests')
	...
	results = evaluate(args.model, True, args.topn)
